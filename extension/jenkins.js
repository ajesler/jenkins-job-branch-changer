...
$(document).ready(function(){
  ...
  jenkins.serverAvailable()
    .done(function(canReachJenkins){
      ...
      if(canReachJenkins) {
        // enable branch update form
        jenkins.jobNames().then(function(jobNames){
          $('#messages').text("Jobs: "+jobNames.join(", "));
        })
      } else {
        $('#messages').text("Cant reach the server, please check your settings.");
      }
    }
  );
  ...
  $('#updateButton').click(function(){
    ...
    jenkins.updateAllJobsBranch(newBranch).done(function(){
      var args = Array.prototype.slice.call(arguments);
      $('#messages').text("Updated jobs "+args.join(", ")+" to use branch "+newBranch);
